...
1) ParsSNP_resources.Rdata
	...
	1) PanCan is the pan-cancer dataframe. Columns 1-24 are the fully scaled and imputed descriptors. The remaining columns indicate whether a mutation belonged in the Training or Test sets, the Sample ID, the Cancer Type, and basic mutation annotations. The ParsSNP column represents scores generated by the ParsSNP model for the test sets, and ParsSNP labels (from the learning phase) for the training mutations. PanCan does NOT include mutations found in hypermutators (see Pan_Cancer_Hypermuators.Rdata, below). 
